...
function signTex(text, o = {}) {
  ...
  return tex(w, h, (g) => {
    ...
    g.font = o.font || `700 ${Math.round(h * 0.5)}px Karla, Arial, sans-serif`;
...
/* ---------------- kit de construcción ---------------- */
function kit(root) {
  const put = (m, x = 0, y = 0, z = 0, ry = 0) => { m.position.set(x, y, z); m.rotation.y = ry; root.add(m); return m; };
  const K = {
    put,
...
const PLACES = {
  ...
  observatorio(K, W) {
    ...
    for (let i = 0; i < 500; i++) { const th = Math.random() * Math.PI * 2, ph = Math.random() * 0.45 * Math.PI; pts.push(40 * Math.cos(th) * Math.sin(ph), 40 * Math.cos(ph) + 2, 40 * Math.sin(th) * Math.sin(ph) - 10); }
    stars.setAttribute("position", new THREE.Float32BufferAttribute(pts, 3));
    K.put(new THREE.Points(stars, new THREE.PointsMaterial({ color: 0xffffff, size: 0.18, sizeAttenuation: true, fog: false })));
  ...
  mall(K, W) {
    K.base(10, 7, "#8d9096", "#4f5257", tiles("parking", "#8f9298", "#e8e8e8", 5), 2);
...
export class World {
  constructor(container, overlay) {
    ...
    this.lineup.add(new THREE.HemisphereLight(0xffffff, 0x223, 1.6));
    const ls = new THREE.DirectionalLight(0xffffff, 1.6); ls.position.set(0, 6, 6); this.lineup.add(ls);
  ...
  homes() {
    ...
    this.home.forEach((p, a) => { a.position.copy(p); a.rotation.set(0, 0, 0); a.visible = true; });
  ...
  resize() {
    ...
    // en pantallas angostas la cámara se aleja para que entre la escena
    const ar = w / h;
  ...
  setPlace(key) {
    ...
    this.place.clear();
    this.pets = [];
    this.evidence = null; this.train = null; this.bus = null; this.flag = null;
    const K = kit(this.place);
    const pal = PLACES[key](K, this);
    this.scene.background = new THREE.Color(pal.sky);
    this.scene.fog = new THREE.Fog(pal.fog, 16, 42);
    this.hemi.color.set(pal.hemi[0]); this.hemi.groundColor.set(pal.hemi[1]);
    this.sun.color.set(pal.sun); this.sun.intensity = pal.sunI; this.baseSun = pal.sunI;
    (pal.pets || []).forEach(p => { this.place.add(p); this.pets.push(p); });
    this.pal = pal;
    this.homes();
    this.villain.visible = false; this.villainLight.intensity = 0;
    if (this.evidence) {
      const ring = new THREE.Mesh(new THREE.TorusGeometry(0.35, 0.03, 8, 32), new THREE.MeshBasicMaterial({ color: 0xf2b134, transparent: true, opacity: 0.9 }));
  ...
  // corre el encuadre hacia arriba para dejar lugar a la interfaz abajo
  ...
  intro({ villain = false } = {}) {
    ...
    this.camPos.set(0, 11, 20); this.camLook.set(0, 0, -1);
    this.sway = 0;
    [this.thomas, this.rocio, this.juli, this.romero].forEach((a, i) => {
      const h = this.home.get(a); a.position.set(h.x, 6 + i, h.z);
      tl.to(a.position, { y: 0, duration: 0.7, ease: "bounce.out" }, 1.2 + i * 0.18);
    });
    tl.to(this.camPos, { x: 2.5, y: 4.2, z: 10, duration: 2.4, ease: "power2.inOut" }, 0);
    tl.to(this.camLook, { x: 0, y: 1, z: 0, duration: 2.4, ease: "power2.inOut" }, 0);
    if (this.train) { this.train.position.x = -14; tl.to(this.train.position, { x: 0, duration: 2.6, ease: "power3.out" }, 0.2); }
    if (this.evidence) tl.to(this.camLook, { x: this.evidence.x * 0.6, y: this.evidence.y, z: this.evidence.z, duration: 1.2, ease: "power2.inOut" }, 2.6)
      .add(() => this.say(null, "Acá pasó algo", 1800, this.evidence), 2.9);
    if (villain) tl.add(() => this.villainCameo(), 4.0).to({}, { duration: 3.2 });
  ...
  villainCameo(text) {
    ...
    gsap.to(this.sun, { intensity: this.baseSun * 0.35, duration: 0.5 });
  ...
  celebrate() {
    ...
    // la patada de kickboxing de Thomas
    gsap.timeline({ delay: 1.9 })
      ...
      .add(() => this.say(this.thomas, "¡Team Bielli!", 1400))
  ...
  defeat() {
    ...
    [this.thomas, this.rocio].forEach(h => { h.userData.busy = true; gsap.to(h.userData.head.rotation, { x: 0.5, duration: 0.8 }); });
    gsap.delayedCall(2.8, () => { [this.thomas, this.rocio].forEach(h => { h.userData.busy = false; gsap.to(h.userData.head.rotation, { x: 0, duration: 0.5 }); }); gsap.to(this.sun, { intensity: this.baseSun, duration: 1 }); });
    this.villainCameo("Jajaja. Miau. Mañana se lo devuelvo. O no.");
  }

  confetti() {
    const n = 160, geo = new THREE.PlaneGeometry(0.08, 0.14);
    const cols = [0xf2b134, 0x4ed39a, 0xff6166, 0x7aa2ff, 0xffffff];
    const mat = new THREE.MeshBasicMaterial({ side: THREE.DoubleSide, vertexColors: false });
    const inst = new THREE.InstancedMesh(geo, mat, n);
    const data = [];
    const m = new THREE.Matrix4(), c = new THREE.Color();
    for (let i = 0; i < n; i++) {
      data.push({ p: V((Math.random() - 0.5) * 6, 5 + Math.random() * 3, 1 + Math.random() * 2), v: V((Math.random() - 0.5) * 1.5, -1.2 - Math.random() * 1.5, 0), r: Math.random() * 6, s: 2 + Math.random() * 6 });
    ...
    this.running.push(dt => {
      life -= dt;
      data.forEach((d, i) => { d.p.addScaledVector(d.v, dt); d.r += d.s * dt; e.set(d.r, d.r * 0.7, 0); q.setFromEuler(e); m.compose(d.p, q, sc); inst.setMatrixAt(i, m); });
  ...
  juliHint(text) {
    const j = this.juli; const u = j.userData;
    u.busy = true;
    const home = this.home.get(j);
    gsap.timeline({ onComplete: () => { u.busy = false; } })
      .to(j.position, { x: 0, z: 3.4, duration: 0.8, ease: "power1.inOut" })
      .to(j.position, { y: 0.4, duration: 0.18, yoyo: true, repeat: 1 })
      .add(() => this.say(j, text, 2600))
      .to(j.position, { x: home.x, z: home.z, duration: 0.9, ease: "power1.inOut" }, "+=2.4");
  }

  romeroRun(ms, onTap) {
    const r = this.romero; const u = r.userData;
    u.busy = true;
  ...
  endRomero(caught) {
    ...
    if (caught) this.say(r, "¡Lo agarraron! Traía una pista en la boca.", 1800);
  ...
  onPointer(e) {
    ...
    // margen generoso: una esfera invisible alrededor de Romero
    const center = this.romero.position.clone(); center.y += 0.4;
    const hit = this.ray.ray.distanceToPoint(center) < 0.75;
    if (hit) { const cb = this.romeroChase.onTap; this.endRomero(true); cb && cb(); }
  }
  ...
  say(actor, text, ms = 2000, point) {
    ...
    const b = { el, actor, point, until: performance.now() + ms };
    this.bubbles.push(b);
  ...
  updateBubbles() {
    ...
    this.bubbles = this.bubbles.filter(b => {
      ...
      const x = Math.max(70, Math.min(w - 70, (p.x * 0.5 + 0.5) * w)), y = Math.max(30, (-p.y * 0.5 + 0.5) * h);
  ...
  buildLineup(sus, wrong = []) {
    ...
    const wall = new THREE.Mesh(new THREE.PlaneGeometry(w, 2.6 / 1), new THREE.MeshBasicMaterial({ map: ht }));
    wall.scale.y = 1; wall.position.set(0, 1.3, -0.6);
    wall.geometry = new THREE.PlaneGeometry(w, 2.6);
  ...
  /* ---- retratos 2D a partir de los modelos 3D ---- */
  portraits(list) {
    if (!this.pr) {
      this.pr = new THREE.WebGLRenderer({ antialias: true, alpha: true, preserveDrawingBuffer: true });
      this.pr.setSize(160, 160); this.pr.setPixelRatio(1); this.pr.outputColorSpace = THREE.SRGBColorSpace;
  ...
  frame() {
    ...
    if (this.lineupActors && this.view === "lineup") this.lineupActors.forEach(a => A.animate(a, t, dt));
    ...
    if (this.view === "lineup") {
      ...
      this.camera.position.set(Math.sin(t * 0.3) * 0.3, 1.1, dist);
      this.camera.lookAt(0, 1.0, 0);
      this.renderer.render(this.lineup, this.camera);
    } else {
      const f = this.fit || 1;
      const sw = this.sway;
      this.camera.position.set(this.camPos.x + Math.sin(t * 0.25) * 0.5 * sw, this.camPos.y + (f - 1) * 1.6, this.camPos.z * f);
